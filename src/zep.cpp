...
{
#ifdef ZEP_CONSOLE
    spZep = std::make_shared<ImGui::ZepConsole>(Zep::ZepPath(std::filesystem::current_path()));
#else
    // Initialize the editor and watch for changes
    spZep = std::make_shared<ZepWrapper>(std::filesystem::current_path(), Zep::NVec2f(pixelScale.x, pixelScale.y), [](std::shared_ptr<ZepMessage> spMessage) -> void {
    });
#endif
